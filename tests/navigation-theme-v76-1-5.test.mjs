...
test('all work pages load the v76.1.5 theme after every legacy stylesheet',()=>{
  for(const page of workPages){
    ...
    assert.match(html,/schoolslogi-theme-v76-1-5\.css\?v=76153/,page+': compact-theme cache key');
    assert.match(html,/professional-shell\.js\?v=7616/,page+': shell cache key');
  }
});
...
test('v76.1.6 search alignment audit includes before and after desktop/mobile screenshots',()=>{
  const directory=resolve(root,'docs','design-v76-1-5','audit-v76-1-6');
