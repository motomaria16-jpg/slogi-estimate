(function(){
  ...
  const snapshot=new Date().toISOString(),freshness=new Date(Date.now()-24*60*60*1000).toISOString(),freshnessCutoff=new Date(new Date(snapshot).getTime()-30*24*60*60*1000).toISOString();
  const listing=(id,address,latitude,longitude,clusterName='')=>({source:'cian',externalId:String(id),listingUrl:`https://www.cian.ru/rent/commercial/${id}`,title:`Помещение ${id}`,address,latitude,longitude,area:100+Number(id),rentMonthly:300000+Number(id)*1000,floor:1,totalFloors:5,ceilingHeight:3.2,firstSeenAt:new Date(new Date(snapshot).getTime()-Number(id)*1000).toISOString(),freshnessAt:freshness,freshnessKind:'published',publishedAt:freshness,marketStatus:'active',clusterName,parseCompleteness:1,parseWarnings:[]});
  ...
  window.fetch=async function(url,options={}){
    ...
    if(target.includes('/search-listings')){const page=Number(body.page)||1,items=pages[page]||[],hasMore=page===1,last=items[items.length-1];return json({items,meta:{sources:{cian:{status:'ok',lastSucceededAt:snapshot}},page,limit:Number(body.limit)||50,total:53,returned:items.length,hasMore,nextPage:hasMore?2:null,nextCursor:hasMore?{firstSeenAt:last.firstSeenAt,source:last.source,listingUrl:last.listingUrl}:null,snapshotAt:snapshot,freshnessCutoff}});}
  ...
  class Clusterer{constructor(){this.items=[];}add(items){this.items.push(...items);window.__slogiFixtureMarkerCount=this.items.length;const node=window.__slogiFixtureMapNode;if(node){node.dataset.fixtureMarkerCount=String(this.items.length);items.slice(0,3).forEach((item,index)=>{const button=document.createElement('button');button.type='button';button.className='fixture-map-marker';button.setAttribute('aria-label',`Тестовый маркер ${index+1}`);button.textContent=`● ${index+1}`;button.addEventListener('click',()=>item.events.emit('click'));node.appendChild(button);});}}removeAll(){this.items=[];window.__slogiFixtureMarkerCount=0;const node=window.__slogiFixtureMapNode;if(node){node.dataset.fixtureMarkerCount='0';node.querySelectorAll('.fixture-map-marker').forEach(item=>item.remove());}}getBounds(){if(!this.items.length)return null;const coords=this.items.map(item=>item.geometry.getCoordinates());return[[Math.min(...coords.map(point=>point[0])),Math.min(...coords.map(point=>point[1]))],[Math.max(...coords.map(point=>point[0])),Math.max(...coords.map(point=>point[1]))]];}}
