(function(root,factory){
  ...
  function filterAndSort(items,criteria={},now=Date.now()){
    ...
    return items.filter(item=>{
      ...
      const clusterMatch=!cluster||(cluster==='__unassigned'?!item.clusterId:item.clusterId===cluster);
  ...
  async function loadAllPages(fetchPage,{limit=100}={}){
    const pageSize=Math.max(1,Math.min(100,Math.trunc(number(limit)||100)));
    const unique=new Map();const seenPages=new Set();
    let page=1,snapshotAt=null,serverTotal=null,meta=null,partial=false,errorCode=null;
    for(;;){
      ...
      catch(error){if(page===1)throw error;partial=true;errorCode='page_failed';break;}
      ...
      meta=result.meta;
      if(page===1)snapshotAt=String(meta.snapshotAt||'');
    ...
    return {items,total:partial?(serverTotal??items.length):items.length,serverTotal,partial,errorCode,meta,pages:seenPages.size};
  }
