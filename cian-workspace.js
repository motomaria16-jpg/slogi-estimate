(function(){
  ...
  function safeListingUrl(value){
    try{const url=new URL(String(value||''));return['http:','https:'].includes(url.protocol)?url.href:'';}catch(_err){return'';}
  }
  ...
  function cardForProject(project,listing){
    ...
    const context=cluster.matched&&service&&typeof service.spaceContext==='function'?service.spaceContext(cluster.id,cluster.name,project.id):{cluster:Object.assign(cluster,{hasSlogiCenter:cluster.status==='outside'?false:null,centerDetails:''}),competitive:{rating:null,rank:null,isTop30:null,isTop35:null,averageRentPerSqm:null}};
    ...
    const storedGeo=stored.geo&&typeof stored.geo==='object'?stored.geo:{};
    const storedCluster=stored.cluster&&typeof stored.cluster==='object'?stored.cluster:{},storedCompetitive=stored.competitive&&typeof stored.competitive==='object'?stored.competitive:{};
    const effectiveGeo=storedGeo.resolutionSource==='manual'?storedGeo:Object.assign({},storedGeo,{lat:number(project.geo&&project.geo.lat!=null?project.geo.lat:listing&&listing.latitude!=null?listing.latitude:storedGeo.lat),lng:number(project.geo&&project.geo.lng!=null?project.geo.lng:listing&&listing.longitude!=null?listing.longitude:storedGeo.lng)});
    const effectiveCluster=storedCluster.resolutionSource==='manual'?storedCluster:Object.assign({},storedCluster,context.cluster);
    const effectiveCompetitive=storedCompetitive.resolutionSource==='manual'?storedCompetitive:Object.assign({},storedCompetitive,context.competitive);
    return spaceCardModel.normalize(Object.assign({},stored,{id:project.id,source:phase.source==='cian'?'cian':'manual',listingUrl:phase.listingUrl||stored.listingUrl||listing&&listing.listingUrl||'',address:project.address,geo:effectiveGeo,cluster:effectiveCluster,competitive:effectiveCompetitive,rentMonthly:phase.rent&&phase.rent.amount,area:project.area,ceilingHeight:project.ceilingHeight,work}));
  }
  ...
  function cardForListing(item){
    ...
    const context=baseCluster.status==='inside'&&service&&typeof service.spaceContext==='function'?service.spaceContext(baseCluster.id,baseCluster.name):{cluster:baseCluster,competitive:baseCluster.name&&service&&typeof service.competitiveProfile==='function'?service.competitiveProfile(baseCluster.id,baseCluster.name):{rating:null,rank:null,isTop30:null,isTop35:null,averageRentPerSqm:null}};
    return spaceCardModel.normalize({id:freshnessId(item),source:'cian',listingUrl:item.listingUrl,address:item.address,geo:{lat:item.latitude,lng:item.longitude,resolutionSource:item.coordinateSource==='manual'?'manual':item.latitude!=null&&item.longitude!=null?'automatic':null},cluster:Object.assign({},baseCluster,context.cluster),competitive:context.competitive,rentMonthly:item.rentMonthly,area:item.area,separateEntrance:null,hasWindows:null,windowsOpen:null,ceilingHeight:item.ceilingHeight,ceilingHeightConfirmed:null,repair:null});
  }
  ...
  function draftForSave(item,cardData,geo,project){
    const phase=project&&project.phase0||{},listingUrl=safeListingUrl(cardData.listingUrl||item&&item.listingUrl||phase.listingUrl);
  ...
  async function saveSpace(item,draft,geo){
    ...
    const project=item&&item._projectId&&projectRepository().get(item._projectId)||item&&existingProject(item),cardData=spaceCardModel.normalize(Object.assign({},draft,{id:project&&project.id||draft.id,source:item&&item.source||draft.source})),manualGeo=cardData.geo&&Number.isFinite(cardData.geo.lat)&&Number.isFinite(cardData.geo.lng)?{lat:cardData.geo.lat,lng:cardData.geo.lng}:null,resolvedGeo=['inside','outside'].includes(cardData.cluster.status)?(manualGeo||geo):null;
  ...
  function openListing(id,trigger){
    ...
    const callbacks={
      onResolveAddress:async draft=>{const service=phase0Service();if(!service||typeof service.resolveSpaceAddress!=='function')throw new Error('Определение кластера временно недоступно.');const result=await service.resolveSpaceAddress(draft.address,current._projectId||'');geo=result.geo;return{address:result.address,geo:result.geo?{lat:result.geo.lat,lng:result.geo.lng,resolutionSource:'automatic'}:null,cluster:result.cluster,competitive:result.competitive};},
  ...
  function openManualSpace(){
    const item={source:'manual',externalId:'',listingUrl:'',title:'Помещение, добавленное вручную',address:'',latitude:null,longitude:null,area:null,rentMonthly:null,ceilingHeight:null,clusterStatus:'not_computed'},initial=spaceCardModel.normalize({source:'manual',listingUrl:'',geo:{lat:null,lng:null},cluster:{status:'not_computed',matched:false,hasSlogiCenter:null},competitive:{rating:null,rank:null,averageRentPerSqm:null},separateEntrance:null,hasWindows:null,windowsOpen:null,ceilingHeightConfirmed:null,repair:null});let geo=null,current=item;
    spaceCardModal.open({initial,onResolveAddress:async draft=>{const result=await phase0Service().resolveSpaceAddress(draft.address);geo=result.geo;return{address:result.address,geo:result.geo?{lat:result.geo.lat,lng:result.geo.lng,resolutionSource:'automatic'}:null,cluster:result.cluster,competitive:result.competitive};},onSave:async draft=>{const saved=await saveSpace(current,draft,geo);current=projectToItem(saved);toast('Помещение добавлено в общий список.');return{card:cardForProject(saved)};},onTakeToWork:async draft=>{const saved=await takeSpace(current,draft,geo);current=projectToItem(saved);return{card:cardForProject(saved)};}});
  }
