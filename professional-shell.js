...
const toolGroups=[
  ...
  {label:'Анализ',links:[['analytics.html','Аналитика']]},
...
function normalizeHeader(){
  ...
  header.dataset.route=productSection()||'tools';
...
function mobileMenuHtml(section){
  ...
  const groups=toolGroups.map(group=>{const links=group.links.filter(([file])=>P.can(file,'view'));return links.length?`<div class="pro-mobile-menu-group"><span>${esc(group.label)}</span>${links.map(([file,text])=>`<a href="${file}" class="${isPage(file)?'active':''}">${esc(text)}</a>`).join('')}</div>`:''}).join('');
  return`<nav aria-label="Мобильная навигация"><div class="pro-mobile-primary">${primary}</div><div class="pro-mobile-tools">${groups}</div></nav>`;
}
