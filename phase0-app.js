...
function renderList(){
  ...
  node.innerHTML=state.visible.length?state.visible.map((project,index)=>cardHtml(project,index)).join(''):`<div class="phase0-empty"><div><strong>${state.projects.length?'По фильтрам ничего не найдено':'Пока нет помещений'}</strong><p>${state.projects.length?'Измените или сбросьте фильтры.':'Добавьте первое помещение вручную или вставьте ссылку на объявление ЦИАН или Авито.'}</p><button class="phase0-btn primary" type="button" data-action="add-project">＋ Добавить объект</button></div></div>`;
  ...
  return`<form id="phase0-object-form" novalidate>
    ...
    <div class="phase0-drawer-body phase0-card-editor-body">
      ...
      <div class="phase0-card-middle-grid phase0-reference-middle">
        ...
        <section class="phase0-card-subpanel phase0-readiness-visual phase0-reference-readiness"><div class="phase0-subpanel-title"><h3>Готовность к смете</h3><span id="phase0-progress-label">Выполнено: ${progress.done} из ${progress.total}</span></div><div class="phase0-progress-content"><div class="phase0-progress-ring" id="phase0-progress-ring" style="--progress:${percent}%"><strong id="phase0-progress-percent">${percent}%</strong></div><div class="phase0-readiness-copy"><div id="phase0-requirements" class="phase0-requirements"></div><button class="phase0-btn phase0-readiness-button" id="phase0-editor-phase1-inline" type="button" data-action="editor-phase1" disabled>Перейти к смете</button></div></div></section>
      </div>
...
function updateEditorReadiness(){
  ...
  const ring=byId('phase0-progress-ring'),pct=byId('phase0-progress-percent'),label=byId('phase0-progress-label');if(ring)ring.style.setProperty('--progress',`${percent}%`);if(pct)pct.textContent=`${percent}%`;if(label)label.textContent=`Выполнено: ${done} из ${checks.length}`;
...
async function importListing(){
  const form=byId('phase0-object-form'),url=fieldValue(form,'listingUrl').trim(),button=byId('phase0-import-button'),message=byId('phase0-import-state');if(!url){setFieldError('listingUrl','Вставьте ссылку ЦИАН или Авито.');form.elements.listingUrl.focus();return}
